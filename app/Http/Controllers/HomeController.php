...
class HomeController extends Controller
{
    ...
    {
        $comprasmes=DB::select('SELECT monthname(i.fecha_hora) as mes, sum(di.cantidad*di.precio_costo) as totalmes from ingreso i inner join detalle_ingreso di on i.idingreso=di.idingreso where i.estado="A" group by monthname(i.fecha_hora) order by month(i.fecha_hora) desc limit 12');
        ...
        $totales=DB::select('SELECT (select ifnull(sum(di.cantidad*di.precio_costo),0) from ingreso i inner join detalle_ingreso di on i.idingreso=di.idingreso where DATE(i.fecha_hora)=curdate() and i.estado="A") as totalingreso, (select ifnull(sum(v.total_venta),0) from venta v where DATE(v.fecha_hora)=curdate() and v.estado="A") as totalventa');
