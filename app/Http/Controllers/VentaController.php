...
class VentaController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request)
        {
           ...
           $ventas=DB::table('venta as v')
            ...
            ->groupBy('v.idventa','v.fecha_hora','c.nombre','v.direccion','v.nit','v.estado')
            ->paginate(7);
    ...
    public function create($id_sucursal)
    {
        ...
        $inventarios = DB::table('inventarios as inv')
            ...
            ->select(DB::raw('CONCAT(clientes.idcliente, " ",clientes.nombre) as clientes'))
          
             // ->select(DB::raw('CONCAT(clientes.idcliente, " ",cliente.nombre) as clientes'),'clientes.direccion',DB::raw('avg(clientes.nit) as nit'))           

            ->select(DB::raw('CONCAT(inv.idarticulo, " ",inv.descripcion) as inventarios'),'inv.idarticulo','inv.cantidad','inv.preciocosto',DB::raw('avg(inv.precioventa) as precio_venta'))
            ...
            ->get();
        return view("ventas.venta.create",["clientes"=>$clientes,"inventarios"=>$inventarios,"sucursales"=>$sucursales/*"empleados"=>$empleados*/]);
    }
    ...
    public function store (VentaFormRequest $request)
    {
           try{
        	...
        	$venta = new Venta;
	        $venta->idcliente=$request->get('pidcliente');
	       // $venta->idsucursal=$request->get('pidsucursal');
            $venta->idempleado=$request->get('pidempleado');
	        $venta->nit=$request->get('pnit');
	        $venta->direccion=$request->get('pdireccion');
	        $venta->total_venta=$request->get('total_venta');
            $venta->ganancia=$request->get('ganancia');
